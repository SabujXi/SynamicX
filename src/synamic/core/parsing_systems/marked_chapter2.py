...
def parse_chapters(text) -> UrlStruct:
    """
    """
    parser = ChaptersParser(text)
    lexer = ChaptersLexer()
    try:
...
class ChaptersParser(Parser):
    def __init__(self, text):
        self.__text = text

    # Get the token list from the lexer (required)
    tokens = ChaptersLexer.tokens
...
if __name__ == '__main__':
    while True:
        ...
        if text:
            try:
                for token in ChaptersLexer().tokenize(text):
                    print(token)
                res = parse_getc(text)
