...
class ContentPath2:
    ...
    def __init__(self, root_absolute_path, relative_path_comps, advance_comp=0, is_file=True, is_meta=False):

        self.__relative_path_comps = relative_path_comps
        self.__root_absolute_path = root_absolute_path
        self.__advance_comp = advance_comp
        self.__is_file = is_file
        self.__is_meta = is_meta
...
class PathTree(object):
    ...
    def __list_paths_loop2(self, starting_components=()):
        ...
        while len(to_travel) != 0:
            ...
            if path.lower().endswith(ContentPath.META_FILE_EXTENSION.lower()):
                ...
                continue
            if os.path.isfile(path_abs):
                path_obj = ContentPath2(absolute_root, path_comps, is_file=True)
                files.append(path_obj)
            elif os.path.isdir(path_abs):
                path_obj = ContentPath2(absolute_root, path_comps, is_file=False)
    ...
    def __list_mod_paths2(self, mod):
        starting_comps = (mod.name,)
    ...
    def __list_mod_files2(self, mod):
        dirs, files = self.__list_mod_paths2(mod)
        return files
    ...
    def __list_mod_dirs2(self, mod):
        ds, fs = self.__list_mod_paths2(mod)
        return ds
