...
class RouterService:
    ...
    def get_content(self, url_str: str):
        ...
        if site is None:
            ...
            # step 1: search for static/binary file in file system with the path components
            # step 2 if 1 fails: search for non-static content and in this case the url is already cached.
            # TODO: add special components processing later.
            url_object = ContentUrl(site, path_components, DocumentType.HTML_DOCUMENT)
            return site.object_manager.get_content_by_url(url_object)

    def make_url(self, site, url_path_comps, for_document_type=None):
